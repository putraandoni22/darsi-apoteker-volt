...
export function SignUpForm() {
	...
	const [role, setRole] = useState<"apoteker" | "pasien">("apoteker");
	...
	return (
		<Card className="w-full border-emerald-200/80 bg-white/95 shadow-[0_18px_32px_-24px_rgba(5,150,105,0.45)] dark:border-emerald-900/80 dark:bg-[#0a1510]/95">
			<CardHeader className="space-y-2">
				...
				/>
				<CardTitle className="text-2xl">Daftar Akun DARSI Apoteker</CardTitle>
				<CardDescription>
					Buat akun untuk mulai menggunakan sistem pencarian obat RSI.
				</CardDescription>
			</CardHeader>
			<CardContent>
				<form onSubmit={onSubmit} className="space-y-4">
					...
					<div className="space-y-2">
						...
						<select
							...
							value={role}
							onChange={(event) =>
								setRole(event.target.value as "apoteker" | "pasien")
							}
							className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
						>
							<option value="apoteker">Apoteker</option>
							<option value="pasien">Pasien (User)</option>
						</select>
					</div>
